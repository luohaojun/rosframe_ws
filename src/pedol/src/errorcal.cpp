...
pcl::PointCloud<pcl::PointNormal>::Ptr E_CloudConvert(pcl::PointCloud<pcl::PointXYZ> cloud_in)
{
...
void E_Segmentation(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_in)
{
...
pcl::PointCloud<pcl::PointXYZ>::Ptr E_cluster(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_cl)
{
    pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_out(new pcl::PointCloud<pcl::PointXYZ>);
    E_Segmentation(cloud_cl);
...
void E_downsample(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_in,
...
pcl::PointCloud<pcl::PointXYZ>::Ptr E_cloud_load(new pcl::PointCloud<pcl::PointXYZ>);
pcl::PointCloud<pcl::PointXYZ>::Ptr E_cloud_seg(new pcl::PointCloud<pcl::PointXYZ>);
pcl::PointCloud<pcl::PointXYZ>::Ptr E_cloud_ds(new pcl::PointCloud<pcl::PointXYZ>);
pcl::PointCloud<pcl::PointXYZ>::Ptr E_cloud_rf(new pcl::PointCloud<pcl::PointXYZ>);
pcl::PointCloud<pcl::PointNormal>::Ptr E_cloudNormal_rf(new pcl::PointCloud<pcl::PointNormal>);
pcl::PointCloud<pcl::PointNormal>::Ptr E_cloudNormal_ds(new pcl::PointCloud<pcl::PointNormal>);
pcl::PointCloud<pcl::PointNormal>::Ptr E_object_aligned(new pcl::PointCloud<pcl::PointNormal>);
FeatureCloudT::Ptr E_object_features(new FeatureCloudT);
FeatureCloudT::Ptr E_scene_features(new FeatureCloudT);




namespace pedol_ns {
    class ErrorCalNodeletClass : public nodelet::Nodelet
    {
    private:
    ...
    public:
        ...
        void onInit()
        {
            ...
            if (pcl::io::loadPLYFile("/home/luo/Group_UAV/Lidar_code/rosbag/0908/0908_asc.ply", *E_cloud_rf) < 0)
            {
                PCL_ERROR("Cloudn't read cloud_in file!");
            }
            std::cout << "\nLoaded file "
                      << "0908_asc.ply"
                      << " (" << E_cloud_rf->size() << " points) "  << std::endl;
            pcl::removeNaNFromPointCloud(*E_cloud_rf, *E_cloud_rf, indices);
            // cout<<"cloud_tr size after removeNaN: "<<(*cloud_tr).size()<<endl;
            E_downsample(E_cloud_rf, E_cloud_rf, 35.0);
            std::cout<<"cloud_rf size: "<<E_cloud_rf->points.size()<<endl;
            ...
            //---------scale---------
            for (int i = 0; i < E_cloud_rf->size(); i++)
            {
                p[0] = E_cloud_rf->points[i].x / 1000.0;
                p[1] = E_cloud_rf->points[i].y / 1000.0;
                p[2] = E_cloud_rf->points[i].z / 1000.0;
                p = R_l_w * p + t_l_w;
                E_cloud_rf->points[i].x = p[0];
                E_cloud_rf->points[i].y = p[1];
                E_cloud_rf->points[i].z = p[2];
            }
            ...
            sub_gt = nh.subscribe("/mavros/local_position/odom",10000,&ErrorCalNodeletClass::cb_gt, this);
            pub_rf = nh.advertise<sensor_msgs::PointCloud2>("rf_cloud",24000);
            sub_livox = nh.subscribe<sensor_msgs::PointCloud2>("in", 24000, &ErrorCalNodeletClass::callback, this);
        ...
        void callback(const sensor_msgs::PointCloud2::ConstPtr& msg_livox)
        {
            ...
            pcl::toROSMsg(*E_cloud_rf, msg_rf);
            msg_rf.header.frame_id = "livox_frame";
            pub_rf.publish(msg_rf);


            
            pcl::fromROSMsg(*msg_livox, *E_cloud_load);
            ros::Time tstamp = msg_livox->header.stamp;
            
            pcl::removeNaNFromPointCloud(*E_cloud_load, *E_cloud_ds, indices);
            E_cloud_seg = E_cluster(E_cloud_ds);
            E_Segmentation(E_cloud_ds);

            point_for_draw.clear();
            for(int i = 0; i<E_cloud_seg->points.size(); i++)
            {
                Vector3d p;
                p[0] = E_cloud_ds->points[i].x;
                p[1] = E_cloud_ds->points[i].y;
                p[2] = E_cloud_ds->points[i].z;
                point_for_draw.push_back(p);
            }

            if (E_cloud_seg->points.size()>=150)
            {
                ...
                gicp.setInputSource(E_cloud_seg);
                gicp.setInputTarget(E_cloud_rf);
                gicp.align(*cloud_icp);
                std::cout << "Applied " << 10 << " ICP iteration(s) in "
                          << time.toc() << " ms" << std::endl;
                pose_est = SE3(gicp.getFinalTransformation().cast<double>().topLeftCorner(3, 3),
                ...
                pcl::toROSMsg(*E_cloud_ds, msg_out);
                pub_livox.publish(msg_out);
                drone_pub->PubT_w_i(pose_est.inverse(), tstamp, AgentId_);
...
PLUGINLIB_EXPORT_CLASS(pedol_ns::ErrorCalNodeletClass, nodelet::Nodelet)