...
int main(int argc, char **argv)
{
    ...
    while(ros::ok()){
        ...
        {
            static VcircleTrj Vcircle1(ros::Time::now().toSec(),
                                     touch_point_x,touch_point_y,touch_point_z,
                                     centre_of_circle_x,centre_of_circle_y,centre_of_circle_z,
                                     -2*PI,10,CIRCLE_TRJ_FACING_FIXED);
            Vcircle1.getPose(ros::Time::now().toSec(),pose);
            if(Vcircle1.finished())
            {
                Vcircle1.getEnding(tmpposition_x,tmpposition_y,tmpposition_z,tmporientation_yaw);
        ...
        {
            static VcircleTrj Vcircle2(ros::Time::now().toSec(),
                                     tmpposition_x,tmpposition_y,tmpposition_z,
                                     centre_of_circle_x,centre_of_circle_y,centre_of_circle_z,
                                     2*PI,10,CIRCLE_TRJ_FACING_FIXED);
            Vcircle2.getPose(ros::Time::now().toSec(),pose);
            if(Vcircle2.finished())
            {
                Vcircle2.getEnding(tmpposition_x,tmpposition_y,tmpposition_z,tmporientation_yaw);
